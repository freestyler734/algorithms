 ...
 * В данной реализации используется уопрядоченное хешерирование -
 * алгортм при котором в каждой пробной последовательности элементы отсортированы по возрастанию.
 * Данная реализация подходит для таблиц в которых не так часто исользуется вставка, как поиск и получение элемента,
 * потому что при упорядочных пробных последовательностях, поиск можно производить пока элементы меньше искомого, если больше,
 * то, очевидно его нет в таблице.
 * Данное решение является хорошим дополнением к рассмотренным ранее методам решения коллизий.
 * Здесь используется линейное пробиривание для простоты.
 */
public class OrderedHashingHashTable {
    private int tableSize = 10;
    private int probingConst = 1;
    private Entry[] hashTable;

    public OrderedHashingHashTable() {
    ...
    private boolean putInternal(Entry entry) {
        ...
        if (hashTable[tableIndex] == null) { // если коллизии нет, то вставляем элемент
            ...
            while (tableIndex != probingIndex) {
                ...
                if (hashTable[probingIndex].getKey() == entry.getKey()){ // нашли дубликат ключа (нарушение условий хеш-тпблиц)
                    return false;
                }

                if (hashTable[probingIndex].getKey() > entry.getKey()) { // если нашли больший элемент, то рехешируем его.
                    Entry temp = hashTable[probingIndex];
                    hashTable[probingIndex] = entry;
                    entry = temp;
                }
    ...
    public double get(int key) {
        ...
        if (hashTable[tableIndex] != null) {
            ...
            // не сделаем полный круг и пока не встретим больший элемент.
            int probingIndex = (tableIndex + probingConst) % tableSize;
            while (hashTable[probingIndex] != null &&
                    probingIndex != tableIndex &&
                    hashTable[tableIndex].getKey() <= key) {
    ...
    public void display() {
        for (int i = 0; i < tableSize; i++) {
            System.out.print((hashTable[i] != null ? hashTable[i].getKey() : hashTable[i]) + ", ");
